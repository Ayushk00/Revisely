...
// Fall back to the lite model when the primary one is overloaded (503) or rate-limited (429)
...
for (const modelName of models) {
  try {
    ...
    if (!retryable || modelName === models[models.length - 1]) throw e;
    console.warn(`${modelName} unavailable (${e.status}), trying next model`);
  }
}
